...
void setup()
{
    ...
    //Make sure the sensor is found, you can also 
    //Address with tempsensor.begin(0x19) for example if A0 is set to 1, or tempsensor.begin(0x1A) if A1 is set to 1
    //Address by default is 0x18, this is by not specifying anything
    if(!tempsensor.begin())
    {
...
void loop()
{
    ...
    Serial.print("Shutdown MCP9808...");
    tempsensor.shutdown_wake(1); //Shutdown MSP9808 - power consumption ~0.1 micro Amperes

    delay(2000);

    Serial.print("wake up MCP9808..."); //Wake up MSP9808 - power consumption ~200 micro Ampere
